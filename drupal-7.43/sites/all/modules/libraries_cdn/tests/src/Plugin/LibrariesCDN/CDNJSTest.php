...
use Drupal\libraries_cdn\Plugin\LibrariesCDN\CDNJS;
 ...
 */
class CDNJSTest extends \PHPUnit_Framework_TestCase {
  ...
  protected function setUp() {
    ...
    $this->plugin = new CDNJS(array(), 'cdnjs', array(), $this->drupal7);
  }
  ...
  public function test_isAvailable() {
    ...
    $this->drupal7->shouldReceive('drupal_http_request')
      ...
      ->andReturn(
        array(
          'code' => 200,
          'data' => json_encode(array('results' => array())),
        )
      );
    // This is executed twice to test $configuration['available'].
  ...
  public function test_getLatestVersion() {
    ...
    $this->drupal7->shouldReceive('drupal_http_request')
      ->once()
      ->with($url, array())
      ->andReturn(array('code' => 200, 'data' => '{"name":"ol3","filename":"ol.min.js","version":"3.8.2"}'));
  ...
  public function test_getVersions() {
    ...
    $results = array(
      'assets' => array(
        array('name' => 'jquery', 'files' => array('test1.js', 'test1.css'), 'version' => '1.0'),
        array('name' => 'jquery', 'files' => array('test2.js', 'test2.css'), 'version' => '2.0'),
        array('name' => 'jquery', 'files' => array('test3.js', 'test3.css'), 'version' => '3.0'),
      ),
    );
    ...
    // Test 3
    $this->plugin->setLibrary('jquery');
    $this->plugin->setURL('isAvailable', 'http://api.cdnjs.com/fake_url?search=%s');
    $urls = $this->plugin->getConfiguration('urls');
